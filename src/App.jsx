...
const RARITY_CFG = {
  R:   { color:"#60a5fa", bg:"linear-gradient(135deg,#1e3a5f,#1e40af)", border:"#3b82f6", glow:"rgba(59,130,246,0.3)" },
  SR:  { color:"#c084fc", bg:"linear-gradient(135deg,#4a1d7a,#7c3aed)", border:"#8b5cf6", glow:"rgba(139,92,246,0.4)" },
  SSR: { color:"#fbbf24", bg:"linear-gradient(135deg,#78350f,#d97706)", border:"#f59e0b", glow:"rgba(251,191,36,0.5)" },
};

function pullOne() {
  const r = Math.random();
  const rarity = r < 0.03 ? "SSR" : r < 0.20 ? "SR" : "R";
  const pool = CARD_POOL.filter(c => c.rarity === rarity);
  return pool[Math.floor(Math.random() * pool.length)];
}

function pullTen() {
  const res = Array.from({ length: 10 }, () => pullOne());
  if (!res.some(c => c.rarity !== "R")) {
    const sr = CARD_POOL.filter(c => c.rarity === "SR" || c.rarity === "SSR");
    res[9] = sr[Math.floor(Math.random() * sr.length)];
  }
  return res;
}
...
function getCardStats(card, level) {
  ...
  return { atk: Math.floor(card.base.atk * m), def: Math.floor(card.base.def * m), spd: Math.floor(card.base.spd * m), hp: Math.floor(card.base.hp * m) };
}
...
const SKILLS = [
  { id:"king_wrath",name:"王之怒",desc:"被動技能，憤怒時攻擊力+3，同時興奮值提升",lv:5,type:"passive",icon:"🔥" },
  { id:"iron_legs",name:"鋼鐵之腿",desc:"下肢力量大幅強化，跳躍力永久+10",lv:10,type:"passive",icon:"🦵" },
  { id:"dominator_hand",name:"支配者之手",desc:"擁有很強的握力，能通過握力鎖死敵人手腕",lv:15,type:"active",icon:"✊" },
  { id:"shadow_step",name:"幻影步",desc:"移動速度大幅提升，敵人難以捕捉你的身影",lv:25,type:"active",icon:"👻" },
  { id:"berserker",name:"狂戰士",desc:"HP低於30%時，所有能力值翻倍",lv:35,type:"passive",icon:"⚡" },
  { id:"titan_body",name:"巨人之軀",desc:"身體硬度超越鋼鐵，物理攻擊幾乎無效",lv:50,type:"passive",icon:"🛡️" },
  { id:"dragon_fist",name:"龍拳",desc:"將全身力量集中在拳頭，一擊粉碎一切",lv:65,type:"active",icon:"🐉" },
  { id:"god_speed",name:"神速",desc:"速度突破人類極限，世界在你眼中靜止",lv:80,type:"active",icon:"⚡" },
  { id:"awakening",name:"覺醒",desc:"超越人類的終極形態，所有能力值無上限突破",lv:100,type:"passive",icon:"👑" },
];

function xpFor(l){return l<=1?0:Math.floor(80*Math.pow(l,1.6));}
function shuffleSeed(a,s){const b=[...a];let x=s;for(let i=b.length-1;i>0;i--){x=(x*9301+49297)%233280;const j=Math.floor((x/233280)*(i+1));[b[i],b[j]]=[b[j],b[i]];}return b;}
function getEx(r){return shuffleSeed(ALL_EX,r*7+13).slice(0,4);}
function getRank(l){
  if(l>=100)return{t:"武神",c:"#ffd700"};if(l>=80)return{t:"傳說",c:"#ff6b35"};
  if(l>=65)return{t:"王者",c:"#ef4444"};if(l>=50)return{t:"霸者",c:"#f97316"};
  if(l>=35)return{t:"英雄",c:"#f59e0b"};if(l>=25)return{t:"勇者",c:"#eab308"};
  if(l>=15)return{t:"鬥士",c:"#84cc16"};if(l>=10)return{t:"挑戰者",c:"#22c55e"};
  if(l>=5)return{t:"見習者",c:"#06b6d4"};return{t:"初心者",c:"#94a3b8"};
}

/* ═══════════════════════════════════════
   SAVE (key = v2 for backward compat)
   ═══════════════════════════════════════ */
...
function CImg({card,sz="full",lvl}){
  ...
  const w=sz==="sm"?76:sz==="md"?110:150;
  const h=Math.floor(w*4/3);
  return(
    <div style={{width:w,height:h,borderRadius:sz==="sm"?8:12,overflow:"hidden",position:"relative",border:`2px solid ${rc.border}`,boxShadow:`0 0 ${sz==="sm"?8:14}px ${rc.glow}`,flexShrink:0}}>
      ...
      <div style={{position:"absolute",top:3,left:3,padding:"1px 5px",borderRadius:3,background:rc.border,color:"#fff",fontSize:sz==="sm"?8:10,fontWeight:900,letterSpacing:1}}>{card.rarity}</div>
      {lvl&&<div style={{position:"absolute",bottom:3,right:3,padding:"1px 5px",borderRadius:3,background:"rgba(0,0,0,0.7)",color:"#fbbf24",fontSize:sz==="sm"?8:10,fontWeight:700}}>Lv.{lvl}</div>}
    </div>
  );
}
...
export default function App(){
  ...
  const xpN=xpFor(gs.level+1),xp0=xpFor(gs.level);
  const xpP=xpN>xp0?((gs.xp-xp0)/(xpN-xp0))*100:100;
  const rank=getRank(gs.level);
  const exs=gs.curEx||getEx(gs.round);
  const allDone=exs.every(e=>(gs.progress[e.id]||0)>=e.target);
  ...
  function doEx(ex){
    const v=parseInt(inp[ex.id])||0;
    if(v<=0){setShkId(ex.id);setTimeout(()=>setShkId(null),500);return;}
    const cur=gs.progress[ex.id]||0,rem=ex.target-cur;
    if(rem<=0)return;
    const reps=Math.min(v,rem),ratio=reps/ex.target;
    const xpG=Math.floor(reps*(2+gs.level*0.15)),gG=Math.floor(reps*0.3);
    ...
    setGs(p=>{
      ...
      const nsk=[...p.skills];let su=null;
      SKILLS.forEach(s=>{if(!nsk.includes(s.id)&&nL>=s.lv){nsk.push(s.id);su=s;}});
      if(lu)setTimeout(()=>{setLvUp(true);setTimeout(()=>setLvUp(false),2500);},200);
    ...
    setInp(p=>({...p,[ex.id]:""}));setGPulse(true);setTimeout(()=>setGPulse(false),500);
  ...
  // ─── GACHA: duplicates only increase copies, NOT level ───
  function doPull(cnt){
    if(gs.gold<cnt){noti("金幣不足！");return;}
    setPulling(true);
    setTimeout(()=>{
      const res=cnt===1?[pullOne()]:pullTen();
      setGs(p=>{
        const nc={...p.cards};
        res.forEach(c=>{
          if(nc[c.id]){nc[c.id]={...nc[c.id],copies:nc[c.id].copies+1};}
          else{nc[c.id]={level:1,copies:0};} // first pull: level=1, 0 spare copies
        });
        return{...p,gold:p.gold-cnt,cards:nc};
      });
      setPullRes(res);setPulling(false);
    },800);
  }

  // ─── MANUAL UPGRADE: consume 1 copy to level up ───
  function upgradeCard(cardId){
    setGs(p=>{
      const cd=p.cards[cardId];
      if(!cd||cd.copies<1)return p;
      const nc={...p.cards,[cardId]:{level:cd.level+1,copies:cd.copies-1}};
      return{...p,cards:nc};
    });
    noti("🔥 角色升級成功！","success");
    // refresh detail
    setCDetail(prev=>prev);
  }
  ...
  const owned=Object.keys(gs.cards).length;
    ...
    <div style={{minHeight:"100vh",minHeight:"100dvh",background:"linear-gradient(175deg,#0a0000 0%,#1a0505 25%,#0d0000 50%,#110808 75%,#0a0000 100%)",color:"#f0e6e6",fontFamily:"'Noto Sans TC',sans-serif",position:"relative",overflow:"hidden",WebkitTapHighlightColor:"transparent",overscrollBehavior:"none"}}>
      ...
      <style>{`
        *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
        ...
        @keyframes lvlAnim{0%{transform:scale(0.2) rotate(-15deg);opacity:0}50%{transform:scale(1.15) rotate(3deg);opacity:1}100%{transform:scale(1) rotate(0);opacity:1}}
        @keyframes skillAnim{0%{transform:scale(0) rotate(-180deg);opacity:0}60%{transform:scale(1.2) rotate(8deg);opacity:1}100%{transform:scale(1) rotate(0);opacity:1}}
        @keyframes borderGlow{0%,100%{box-shadow:0 0 15px rgba(239,68,68,0.3)}50%{box-shadow:0 0 30px rgba(249,115,22,0.5)}}
        @keyframes cardFlip{0%{transform:rotateY(180deg) scale(0.5);opacity:0}60%{transform:rotateY(0) scale(1.1);opacity:1}100%{transform:rotateY(0) scale(1);opacity:1}}
      `}</style>
      ...
      {shop&&!pullRes&&<div style={{position:"fixed",inset:0,zIndex:250,background:"rgba(0,0,0,0.93)",backdropFilter:"blur(12px)",overflow:"auto",WebkitOverflowScrolling:"touch"}}>
        <div style={{maxWidth:460,margin:"0 auto",padding:"max(16px,env(safe-area-inset-top)) 16px calc(20px + env(safe-area-inset-bottom))"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"14px"}}>
            ...
            <button onClick={()=>setShop(false)} style={{background:"none",border:"none",color:"#78716c",fontSize:"24px",cursor:"pointer",padding:"4px"}}>✕</button>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"14px",padding:"8px 14px",borderRadius:"10px",background:"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.2)"}}>
            <span style={{fontSize:"16px"}}>🪙</span><span style={{fontFamily:"'Black Ops One',cursive",color:"#fbbf24",fontSize:"18px"}}>{gs.gold}</span>
          </div>
          <div style={{padding:"18px",borderRadius:"16px",background:"linear-gradient(135deg,rgba(251,191,36,0.06),rgba(239,68,68,0.04))",border:"1px solid rgba(251,191,36,0.15)",marginBottom:"14px",textAlign:"center"}}>
            ...
            <div style={{display:"flex",justifyContent:"center",gap:"6px",marginBottom:"12px"}}>
              {CARD_POOL.filter(c=>c.rarity==="SSR").map(c=>(<CImg key={c.id} card={c} sz="sm"/>))}
            </div>
            <div style={{fontSize:"11px",color:"#a8a29e"}}>SSR 3% ｜ SR 17% ｜ R 80%</div>
            <div style={{fontSize:"10px",color:"#78716c",marginTop:"3px"}}>10抽保底至少1張SR</div>
          </div>
          <div style={{display:"flex",gap:"10px",marginBottom:"18px"}}>
            <button onClick={()=>doPull(1)} disabled={pulling||gs.gold<1} style={{flex:1,padding:"13px",borderRadius:"14px",border:"2px solid rgba(251,191,36,0.4)",background:"linear-gradient(135deg,rgba(251,191,36,0.1),rgba(249,115,22,0.06))",color:"#fbbf24",fontWeight:900,fontSize:"16px",cursor:"pointer",fontFamily:"inherit",opacity:gs.gold<1?0.4:1}}>{pulling?"抽獎中...":"單抽 🪙1"}</button>
            <button onClick={()=>doPull(10)} disabled={pulling||gs.gold<10} style={{flex:1,padding:"13px",borderRadius:"14px",border:"2px solid rgba(239,68,68,0.4)",background:"linear-gradient(135deg,rgba(239,68,68,0.12),rgba(249,115,22,0.08))",color:"#f97316",fontWeight:900,fontSize:"16px",cursor:"pointer",fontFamily:"inherit",opacity:gs.gold<10?0.4:1}}>{pulling?"抽獎中...":"十抽 🪙10"}</button>
          </div>
          <div style={{fontSize:"12px",fontWeight:700,color:"#a8a29e",marginBottom:"8px"}}>[卡池一覽]</div>
          {["SSR","SR","R"].map(r=>(<div key={r} style={{marginBottom:"10px"}}><div style={{fontSize:"10px",fontWeight:700,color:RARITY_CFG[r].color,marginBottom:"4px"}}>{r}</div><div style={{display:"flex",gap:"6px",overflowX:"auto",paddingBottom:"4px",WebkitOverflowScrolling:"touch"}}>{CARD_POOL.filter(c=>c.rarity===r).map(c=>(<div key={c.id} style={{textAlign:"center",flexShrink:0}}><CImg card={c} sz="sm"/><div style={{fontSize:"9px",color:"#a8a29e",marginTop:"3px"}}>{c.name}</div></div>))}</div></div>))}
        </div>
      </div>}

      {/* ═══ PULL RESULT ═══ */}
      {pullRes&&<div style={{position:"fixed",inset:0,zIndex:260,background:"rgba(0,0,0,0.95)",backdropFilter:"blur(12px)",overflow:"auto",WebkitOverflowScrolling:"touch"}}>
        <div style={{maxWidth:460,margin:"0 auto",padding:"max(20px,env(safe-area-inset-top)) 16px calc(20px + env(safe-area-inset-bottom))",textAlign:"center"}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:"8px",marginBottom:"20px"}}>
            {pullRes.map((c,i)=>{
              ...
              const isDupe=gs.cards[c.id]&&gs.cards[c.id].copies>=0;
              return(<div key={i} style={{animation:`cardFlip 0.5s ${i*0.1}s ease-out both`,textAlign:"center"}}>
                <CImg card={c} sz="md"/>
                <div style={{fontSize:"10px",color:rc.color,marginTop:"3px",fontWeight:700}}>{c.name}</div>
                {isDupe&&<div style={{fontSize:"9px",color:"#fbbf24"}}>+1 素材</div>}
              </div>);
            })}
          </div>
          <div style={{display:"flex",gap:"10px",justifyContent:"center"}}>
            <button onClick={()=>{setPullRes(null);}} style={{padding:"10px 30px",borderRadius:"12px",border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.05)",color:"#a8a29e",fontWeight:700,fontSize:"14px",cursor:"pointer",fontFamily:"inherit"}}>繼續抽</button>
            <button onClick={()=>{setPullRes(null);setShop(false);setVw("cards");}} style={{padding:"10px 30px",borderRadius:"12px",border:"2px solid rgba(251,191,36,0.4)",background:"linear-gradient(135deg,rgba(251,191,36,0.1),rgba(249,115,22,0.06))",color:"#fbbf24",fontWeight:900,fontSize:"14px",cursor:"pointer",fontFamily:"inherit"}}>去升級角色 →</button>
          </div>
        </div>
      </div>}

      {/* ═══ CARD DETAIL + MANUAL UPGRADE ═══ */}
      {cDetail&&<div style={{position:"fixed",inset:0,zIndex:250,background:"rgba(0,0,0,0.92)",backdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setCDetail(null)}>
        <div onClick={e=>e.stopPropagation()} style={{animation:"lvlAnim 0.4s ease-out",textAlign:"center",padding:"24px 28px",background:"linear-gradient(135deg,rgba(20,10,10,0.95),rgba(30,15,15,0.95))",border:`2px solid ${RARITY_CFG[cDetail.rarity].border}`,borderRadius:"24px",maxWidth:"300px",width:"90%",boxShadow:`0 0 40px ${RARITY_CFG[cDetail.rarity].glow}`}}>
          <CImg card={cDetail} sz="full" lvl={gs.cards[cDetail.id]?.level}/>
          <div style={{marginTop:"10px",fontSize:"20px",fontWeight:900,color:RARITY_CFG[cDetail.rarity].color}}>{cDetail.name}</div>
          <div style={{fontSize:"12px",color:RARITY_CFG[cDetail.rarity].color,marginTop:"2px"}}>{cDetail.rarity}　Lv.{gs.cards[cDetail.id]?.level||1}</div>
          <div style={{fontSize:"11px",color:"#78716c",marginTop:"2px"}}>升級素材：{gs.cards[cDetail.id]?.copies||0} 個</div>

          {/* Stats */}
          {gs.cards[cDetail.id]&&(()=>{
            const st=getCardStats(cDetail,gs.cards[cDetail.id].level);
            return(<div style={{marginTop:"10px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:"5px"}}>
              {[{k:"atk",l:"攻擊",c:"#ef4444"},{k:"def",l:"防禦",c:"#3b82f6"},{k:"spd",l:"速度",c:"#22c55e"},{k:"hp",l:"血量",c:"#f59e0b"}].map(s=>(
                <div key={s.k} style={{padding:"5px",borderRadius:"7px",background:"rgba(255,255,255,0.03)"}}>
                  <div style={{fontSize:"9px",color:"#78716c"}}>{s.l}</div>
                  <div style={{fontFamily:"'Black Ops One',cursive",fontSize:"17px",color:s.c}}>{st[s.k]}</div>
                </div>
              ))}
            </div>);
          })()}
          ...
          {/* UPGRADE BUTTON */}
          {gs.cards[cDetail.id]&&gs.cards[cDetail.id].copies>=1&&(
            <button onClick={()=>upgradeCard(cDetail.id)} style={{marginTop:"12px",padding:"10px 32px",borderRadius:"12px",border:"2px solid rgba(34,197,94,0.5)",background:"linear-gradient(135deg,rgba(34,197,94,0.15),rgba(22,163,74,0.1))",color:"#4ade80",fontWeight:900,fontSize:"15px",cursor:"pointer",fontFamily:"inherit",animation:"pulse 2s infinite",width:"100%"}}>
              ⬆️ 升級（消耗 1 素材）
            </button>
          )}
          {gs.cards[cDetail.id]&&gs.cards[cDetail.id].copies<1&&(
            <div style={{marginTop:"10px",fontSize:"11px",color:"#57534e"}}>需要抽到重複角色獲得素材才能升級</div>
          )}

          <button onClick={()=>setCDetail(null)} style={{marginTop:"10px",padding:"6px 20px",borderRadius:"8px",border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.03)",color:"#78716c",fontSize:"11px",cursor:"pointer",fontFamily:"inherit"}}>關閉</button>
        </div>
      </div>}
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:460,margin:"0 auto",padding:`max(16px,env(safe-area-inset-top)) 14px calc(90px + env(safe-area-inset-bottom))`}}>
        ...
        <div style={{textAlign:"center",marginBottom:"14px",paddingTop:"4px"}}>
          ...
          <div style={{fontSize:"11px",color:"#78716c",marginTop:"2px"}}>第 {gs.round} 關　|　通關 {gs.cleared}</div>
        </div>
        ...
        <div style={{display:"flex",gap:"10px",marginBottom:"14px"}}>
          ...
          <div onClick={()=>setShop(true)} style={{display:"flex",alignItems:"center",gap:"5px",padding:"2px 12px",borderRadius:"8px",background:"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.2)",cursor:"pointer",animation:gPulse?"goldPop 0.4s ease":"none"}}>
            <span style={{fontSize:"15px"}}>🪙</span><span style={{fontFamily:"'Black Ops One',cursive",color:"#fbbf24",fontSize:"15px"}}>{gs.gold}</span>
          </div>
        </div>
        ...
        {vw==="cards"&&<div style={{animation:"fadeUp 0.3s ease-out"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"8px"}}>
            {CARD_POOL.map(c=>{const o=gs.cards[c.id];return(
              <div key={c.id} onClick={()=>o&&setCDetail(c)} style={{textAlign:"center",opacity:o?1:0.25,cursor:o?"pointer":"default"}}>
                <CImg card={c} sz="sm" lvl={o?.level}/>
                <div style={{fontSize:"10px",color:o?RARITY_CFG[c.rarity].color:"#57534e",marginTop:"3px",fontWeight:700}}>{c.name}</div>
                {o&&o.copies>0&&<div style={{fontSize:"9px",color:"#fbbf24"}}>素材 ×{o.copies}</div>}
              </div>
            );})}
          </div>
          {owned===0&&<div style={{textAlign:"center",padding:"28px",color:"#57534e",fontSize:"13px"}}>還沒有角色<br/><span style={{fontSize:"11px",color:"#78716c"}}>點上方 🪙 去抽獎</span></div>}
        </div>}
        ...
        {vw==="stats"&&<div style={{animation:"fadeUp 0.3s ease-out"}}>
          ...
          <div style={{padding:"18px",borderRadius:"16px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.04)"}}>
            {[{k:"strength",l:"力量",i:"💪",c:"#ef4444"},{k:"stamina",l:"體力",i:"❤️",c:"#22c55e"},{k:"agility",l:"敏捷",i:"⚡",c:"#f59e0b"},{k:"jump",l:"跳躍",i:"🦘",c:"#3b82f6"},{k:"grip",l:"握力",i:"✊",c:"#a855f7"}].map((s,idx)=>{const v=gs.stats[s.k]||0;return(
              <div key={s.k} style={{display:"flex",alignItems:"center",gap:"9px",marginBottom:idx<4?"12px":0}}><span style={{fontSize:"17px",width:"22px",textAlign:"center"}}>{s.i}</span><span style={{width:"40px",fontSize:"11px",color:"#a8a29e"}}>{s.l}</span><div style={{flex:1,height:"6px",borderRadius:"3px",background:"rgba(255,255,255,0.04)",overflow:"hidden"}}><div style={{height:"100%",borderRadius:"3px",width:`${Math.min((v/Math.max(v,40))*100,100)}%`,background:`linear-gradient(90deg,${s.c},${s.c}88)`}}/></div><span style={{fontFamily:"'Black Ops One',cursive",fontSize:"13px",color:s.c,width:"34px",textAlign:"right"}}>{v}</span></div>
            );})}
          </div>
        ...
        {vw==="skills"&&<div style={{animation:"fadeUp 0.3s ease-out"}}>
          ...
          {SKILLS.map((sk,i)=>{const u=gs.skills?.includes(sk.id)||(gs.unlockedSkills||[]).includes(sk.id);return(
